...
class BookingService {
    /**
     * Confirm booking and mark as paid (idempotent)
    ...
    async confirmBooking(bookingId) {
      ...
      // Idempotent: nếu đã confirmed hoặc đã paid thì trả về booking luôn
      if (booking.status === 'confirmed' || booking.payment_status === 'paid' || (booking.payment && booking.payment.status === 'paid')) {
        return booking;
      }
      // Cập nhật trạng thái booking và payment_status luôn, kể cả khi booking.payment là null
      ...
      // Gửi thông báo xác nhận nếu cần
      try {
        await this.sendBookingConfirmation({ ...booking, status: 'confirmed', payment_status: 'paid' });
      } catch (err) {
        console.error('[BookingService] Error sending confirmation notification:', err.message);
      }
      // Lấy lại booking đã cập nhật
  ...
  async confirmPayment(bookingId, paymentData) {
    ...
    // Call payment-service to create payment (MoMo/PayOS)
    ...
    try {
      ...
      // PATCH: Luôn truyền bookingId cho payment-service
      paymentResponse = await axios.post(
        `${paymentServiceUrl}/api/payment`,
        {
          bookingId, // BẮT BUỘC: truyền bookingId
          orderId: orderCode,
      ...
      throw new Error('Failed to initiate payment with payment-service');
    }

    const { paymentUrl, qrCode, clientSecret, provider } = paymentResponse.data || {};
    ...
    // Return booking info + paymentUrl/qrCode + Stripe fields for frontend
    return {
      ...booking,
      paymentUrl,
      qrCode,
...
module.exports.confirmBooking = BookingService.prototype.confirmBooking;
