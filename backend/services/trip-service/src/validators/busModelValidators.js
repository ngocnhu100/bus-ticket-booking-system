    ...
    .items(
      Joi.object({
        seat_number: Joi.string()
          .pattern(/^VIP\d+[A-Z]+|\d+[A-Z]+$/)
          .required(),
        row: Joi.number().integer().min(0).required(),
        col: Joi.number().integer().min(0).required(),
        type: Joi.string().valid('standard', 'vip').default('standard'),
...
const createBusModelSchema = Joi.object({
  ...
  total_seats: Joi.number().integer().min(12).max(60).required().messages({
...
const setSeatLayoutSchema = Joi.object({
  layout_json: seatLayoutSchema.required().messages({
